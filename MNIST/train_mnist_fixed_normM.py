...
def parse_args():
    ...
    parser.add_argument('--M_constraint', type=str, default='none',
                        choices=['none', 'fixed_norm', 'clamp_norm'],
                        help='How to constrain M norm during learning. '
                             'none: L2 regularizer only. '
                             'fixed_norm: ||M||_F = const, learn direction only. '
                             'clamp_norm: project M back if ||M||_F exceeds max.')
    parser.add_argument('--M_norm_radius', type=float, default=None,
                        help='Fixed Frobenius norm for M (fixed_norm mode). '
                             'Defaults to ||M_init||_F.')
    parser.add_argument('--M_max_norm', type=float, default=None,
                        help='Max Frobenius norm for M (clamp_norm mode). '
...
def write_stats_file(path, args, device, total_params, trainable_params,
    ...
    if args.model_type == 'active':
        ...
        lines.append(f"  M_constraint   = {args.M_constraint}")
...
def main(args):
    ...
    if args.model_type == 'active':
        ...
        model = MNIST_Linear_Diffusion(
            ...
            M_constraint=args.M_constraint,
            M_norm_radius=args.M_norm_radius,
            M_max_norm=args.M_max_norm,
        ...
        if args.learn_M:
            print(f"  M learning rate factor: {args.M_lr_factor}, "
                  ...
                  f"constraint: {args.M_constraint}")
    ...
    if args.model_type == 'linear' and args.learn_M:
        # Identify the M parameter (named differently in fixed_norm mode)
        if args.M_constraint == 'fixed_norm':
            M_param = [model._M_raw]
        else:
            M_param = [model.M]
        unet_params = list(model.model.parameters())
        optimizer = AdamW([
            {'params': unet_params, 'lr': args.lr},
            {'params': M_param, 'lr': args.lr * args.M_lr_factor},
        ])
        print(f"Optimizer: U-Net lr={args.lr}, M lr={args.lr * args.M_lr_factor}")
        print(f"  M constraint: {args.M_constraint}")
    else:
    ...
    for i in range(start_epoch, args.epochs):
        ...
        for j, (image, target) in enumerate(train_dataloader):
            ...
            # Apply norm constraint on M if needed
            if args.model_type == 'linear' and args.learn_M:
                model.clamp_M_norm()  # no-op unless M_constraint == 'clamp_norm'
            ...
            if j % args.log_freq == 0:
                ...
                if args.model_type == 'linear' and args.learn_M:
                    M_eff = model.get_M().detach().cpu().numpy()
                    tr_val = M_eff[0, 0] + M_eff[1, 1]
                    det_val = M_eff[0, 0] * M_eff[1, 1] - M_eff[0, 1] * M_eff[1, 0]
                    M_norm = (M_eff ** 2).sum() ** 0.5
                    log_msg += (
                        f",M=[{M_eff[0,0]:.3f},{M_eff[0,1]:.3f};"
                        f"{M_eff[1,0]:.3f},{M_eff[1,1]:.3f}]"
        ...
        if args.model_type == 'linear' and args.learn_M:
            stat["M_values"] = model.get_M().detach().cpu().numpy().round(4).tolist()
        ...
        if args.model_type == 'linear' and args.learn_M:
            ...
            M_norm = (M_eff ** 2).sum() ** 0.5
            summary_msg += (
                f"\n  Learned M = [{M_eff[0,0]:.4f}, {M_eff[0,1]:.4f}; "
                f"{M_eff[1,0]:.4f}, {M_eff[1,1]:.4f}]"
                f"  tr={M_eff[0,0]+M_eff[1,1]:.4f}"
                f"  det={M_eff[0,0]*M_eff[1,1]-M_eff[0,1]*M_eff[1,0]:.4f}"
                f"  ||M||_F={M_norm:.4f}"
            )
        print(summary_msg)
