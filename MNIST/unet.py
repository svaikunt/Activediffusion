...
class DiscreteTimeEmbedding(nn.Module):
    def __init__(self, timesteps, time_embedding_dim):
        super().__init__()
        self.timesteps = timesteps
        self.time_embedding = nn.Embedding(timesteps, time_embedding_dim)

    def forward(self, t):
        # Convert continuous time [0,1] to discrete steps [0, timesteps-1]
        t_discrete = (t * (self.timesteps - 1)).long().clamp(0, self.timesteps - 1)
        return self.time_embedding(t_discrete)
...
class Unet(nn.Module):
    ...
    def __init__(self,timesteps,time_embedding_dim,in_channels=3,out_channels=2,base_dim=32,dim_mults=[2,4,8,16]):
        ...
        # Replace SinusoidalTimeEmbedding with DiscreteTimeEmbedding
        self.time_embedding = DiscreteTimeEmbedding(timesteps, time_embedding_dim)
